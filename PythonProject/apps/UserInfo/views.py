...
class FrontAddView(BaseView):  # 前台用户信息添加
    ...
    def post(self, request):
        ...
        userInfo.money = 0.0 #float(request.POST.get('userInfo.money'))
        userInfo.regTime = str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) # request.POST.get('userInfo.regTime')
...
class FrontListView(BaseView):  # 前台用户信息查询列表
    ...
    def handle(self, request):
        ...
        regTime = self.getStrParam(request, 'regTime')
        # 然后条件组合查询过滤
        ...
        if regTime != '':
            userInfos = userInfos.filter(regTime__contains=regTime)
...
class UpdateView(BaseView):  # Ajax方式用户信息更新
    ...
    def post(self, request, user_name):
        ...
        userInfo.city = request.POST.get('userInfo.city')
        userInfo.address = request.POST.get('userInfo.address')
        userInfo.money = float(request.POST.get('userInfo.money'))
